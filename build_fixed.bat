...
pip freeze | findstr PyQt5 > nul
if %errorlevel% equ 0 (
    echo Temporarily uninstalling PyQt5 to prevent conflicts...
    pip freeze | findstr PyQt5 > pyqt5_packages.txt
    for /F "tokens=*" %%a in (pyqt5_packages.txt) do pip uninstall -y "%%a"
    echo PyQt5 packages uninstalled temporarily.
) else (
    echo No PyQt5 packages found. Continuing with build...
    echo. > pyqt5_packages.txt
)
...
echo Restoring PyQt5 packages if they were uninstalled...
for /F "tokens=*" %%a in (pyqt5_packages.txt) do (
    if not "%%a"=="" (
        echo Installing %%a
        pip install "%%a"
    )
)
del pyqt5_packages.txt
